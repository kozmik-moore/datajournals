...
from datajournals.models import DreamRecord, DreamNote, DreamSubjectTag, DreamDescriptorTag, DreamEmotionTag


def get_dreams_stats(dataframe: pd.DataFrame = None):
    if dataframe is None:
        dq = db.session.query(DreamRecord.id.label('record'),
                              DreamRecord.record_date.label('date')
                              ).order_by(DreamRecord.record_date.desc())
    ...
    if td['all'].shape[0]:
        ...
        ln = db.session.query(DreamRecord).filter_by(id=int(i)).first()
    setattr(s, 'latest', ln)
    return s
...
def get_notes_stats(dataframe: pd.DataFrame = None):
    if dataframe is None:
        nq = db.session.query(DreamNote.id.label('note'),
                              DreamNote.date_added.label('date')).order_by(DreamNote.date_added.desc())
    ...
    ln = None
    if td['all'].shape[0]:
        i = td['all'].reset_index().loc[0, 'note']
        ln = db.session.query(DreamNote).filter_by(id=int(i)).first()
    setattr(s, 'latest', ln)
    return s
...
def get_tags_stats(dataframe: pd.DataFrame = None):
    if dataframe is None:
        nq = (db.session.query(DreamRecord.id.label('record'),
                               DreamRecord.record_date.label('date'),
                               DreamSubjectTag.id.label('subjects'),
                               DreamDescriptorTag.id.label('descriptors'),
                               DreamEmotionTag.id.label('emotions'),
                               )
              .join(DreamRecord.subjects)
              .join(DreamRecord.descriptors)
              .join(DreamRecord.emotions)
              )
        dataframe = pd.read_sql(nq.statement, db.engine)
    atd = {}
    atc = {}
    tc = {}
    for i, m in ('subjects', DreamSubjectTag), ('descriptors', DreamDescriptorTag), ('emotions', DreamEmotionTag):
...
def get_tag_stats(tag: DreamSubjectTag | DreamDescriptorTag | DreamEmotionTag, dataframe: pd.DataFrame = None):
    tm = type(tag)
    tt = {
        DreamSubjectTag: 'subject',
        DreamDescriptorTag: 'descriptor',
        DreamEmotionTag: 'emotion'
    }[tm]
    if dataframe is None:
        tq = (db.session.query(DreamRecord.id.label('record'),
                               DreamRecord.record_date.label('date'),
                               tm.id.label(tt),
                               )
              .join(getattr(DreamRecord, f'{tt}s'))
              )
